...
export function PlayerTriangularHistory({ playerId, playerName }: PlayerTriangularHistoryProps) {
  ...
  const handleTriangularClick = (triangularId: string) => {
    // Navegar al historial con el ID del triangular y el jugador seleccionado
    const url = `/historial?triangularId=${triangularId}&playerId=${playerId}`;
    router.push(url);
  };
    ...
    return (
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <div className="flex justify-center items-center h-24">
          <div role="status" aria-hidden="true" className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-500"></div>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-2">
            {triangulars.map((triangular) => {
              console.log('Triangular:', {
                ...
                playerTeam: triangular.playerTeam,
                teams: triangular.teams,
                teamPlayers: triangular.teamPlayers,
                position: triangular.position,
                points: triangular.points,
                wins: triangular.wins,
                draws: triangular.draws,
                goals: triangular.goals,
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-sm text-gray-300">
                      {getColorByTeam(triangular.playerTeam as "Equipo 1" | "Equipo 2" | "Equipo 3")}
                    </div>
                  </div>
                  ...
                  <div className="mt-3 pt-2 border-t border-gray-600">
                    <div className="flex justify-between items-center text-xs text-gray-400 mb-3">
                      <div>
                        Campeón: <span className="text-yellow-400">{getColorByTeam(triangular.champion as "Equipo 1" | "Equipo 2" | "Equipo 3")}</span>
                      </div>
                      <span 
                          ...
                          <div key={team.teamName} className="bg-gray-800 rounded p-2">
                            <div className="flex items-center justify-between mb-2">
                              <span className={`text-xs font-semibold ${
                                ...
                                {getColorByTeam(team.teamName as "Equipo 1" | "Equipo 2" | "Equipo 3")}
                              </span>
